...
const seed = async () => {
  ...
  const teacherUsers = await Promise.all(
    teacherData.map(t =>
      User.create({ name: t.name, email: t.email, password: 'Teacher@123', role: 'teacher', phone: t.phone })
    )
  );
  ...
  await Promise.all(
    teacherUsers.map((u, i) =>
      User.findByIdAndUpdate(u._id, { teacherProfile: teacherProfiles[i]._id })
    )
  );
  ...
  await Promise.all(
    studentUsers.map((u, i) =>
      User.findByIdAndUpdate(u._id, { studentProfile: studentProfiles[i]._id })
    )
  );
  ...
  await Announcement.insertMany([
    { title: '🏆 Annual Sports Day 2025 — December 15', content: 'Annual Sports Day on December 15, 2025. Events include 100m sprint, relay race, long jump, basketball and cricket. All students must register by December 10.', type: 'event', targetAudience: ['all'], author: adminUser._id, isPinned: true },
    { title: '📝 Mid-Term Examination Schedule Released', content: 'Mid-term examinations for all classes will be held from November 20–30, 2025. Students are advised to start preparation immediately.', type: 'exam', targetAudience: ['student', 'parent'], author: adminUser._id },
    { title: '💰 Q3 Fee Payment — Last Date November 30', content: 'Last date for Q3 fee payment is November 30, 2025. Parents can pay online via the parent portal. Late payment will attract a fine of ₹500.', type: 'fee', targetAudience: ['parent', 'student'], author: adminUser._id },
    { title: '📚 New Library Books — 200 Titles Added', content: 'The school library has received 200 new books across Science, Mathematics, Literature and General Knowledge.', type: 'general', targetAudience: ['all'], author: adminUser._id },
    { title: '🎨 Inter-School Art Competition', content: 'EduManage School is hosting the Regional Inter-School Art Competition on December 5, 2025. Register with the Art Department by November 25.', type: 'event', targetAudience: ['student'], author: adminUser._id },
    { title: '🚨 URGENT: School Closed Tomorrow', content: 'Due to heavy rainfall warning, the school will remain closed tomorrow. Online classes will be conducted as per the regular timetable.', type: 'urgent', targetAudience: ['all'], author: adminUser._id, isPinned: true },
  ]);
  ...
  await Payment.insertMany(
    studentProfiles.slice(0, 5).map((s, i) => ({
      ...
      dueAmount:     [0, 15000, 7500, 0, 15000][i],
  ...
  for (const day of days.slice(0, 3)) {
    for (let si = 0; si < slots.length; si++) {
      ttEntries.push({
        class:        classes[0]._id,
        subject:      subjects[si % subjects.length]._id,
  ...
  console.log(`✅ ${ttEntries.length} timetable entries created`);
  ...
  const salaryDocs = [];
  for (const [ti, teacher] of teacherProfiles.entries()) {
    for (const [mi, month] of months.entries()) {
      const base = teacherData[ti].sal;
      salaryDocs.push({
        employee:       teacherUsers[ti]._id,
        ...
        deductions:     Math.round(base * 0.12),
  ...
  await Homework.insertMany([
    {
      ...
      description: 'Solve all exercises from Chapter 5 (pages 112–118). Show all working steps.',
    ...
    },
    {
      ...
      description: 'Write a 500-word essay on your favourite book. Include author, plot summary, and why you recommend it.',
    ...
    },
    {
      ...
      description: 'Write a complete lab report for the acid-base titration experiment. Include observations, calculations and conclusions.',
    ...
    },
    {
      ...
      description: 'Write a Python program that performs +, -, *, /. Handle division by zero gracefully.',
